...
NCountry = {
	...
	NAVY_SCORE_MULTIPLIER = 5.0,					-- Based on number of navies.
	AIR_SCORE_MULTIPLIER = 0.03,					-- Based on number of planes (which is typically a lot).
	...
	NUCLEAR_PRODUCTION_SCALE = 0,					-- +1 nuclear_production gives 1 nuke per year
	NUCLEAR_BOMB_DROP_WAR_SUPPORT_EFFECT_MAX_INFRA = 0.25, -- Reduce enemy national war support on nuking a province, the value scales with infrastructure up to this number
...
NProduction = {
	...
	BASE_FACTORY_MAX_EFFICIENCY_FACTOR = 40,	-- Base max efficiency for factories expressed in %.
...
NMilitary = {
	...
	OUT_OF_SUPPLY_ATTRITION = 0.15,                 -- max attrition when out of supply
	OUT_OF_SUPPLY_SPEED = -0.65,                    -- max speed reduction from supply
	...
	NUKE_MIN_DAMAGE_PERCENT = 0.8,					-- Minimum damage from nukes as a percentage of current strength/organisation
...
NNavy = {
	...
	DETECTION_CHANCE_MULT_BASE = 0.15,								-- base multiplier value for detection chance. Later the chance is an average between our detection and enemy visibility, mult by surface/sub detection chance in the following defines.
	...
	MAX_CAPITALS_PER_AUTO_TASK_FORCE = 3,							-- maximum number of capital ships the auto-task force creation will put together when designing SurfaceActionGroup
	MAX_SUBMARINES_PER_AUTO_TASK_FORCE = 12,						-- maximum number of submarines the auto-task force creation will put together when designing wolfpack
	BEST_CAPITALS_TO_CARRIER_RATIO = 1,							-- capitals / carriers ratio used when auto-task force creation designs CarrierTaskForce
	BEST_CAPITALS_TO_SCREENS_RATIO = 0.20, 							-- capitals / screens ratio used for creating FEX groups in naval combat
	COMBAT_BASE_HIT_CHANCE = 0.15,									-- base chance for hit
	...
	CARRIER_STACK_PENALTY = 3,										-- The most efficient is 4 carriers in combat. 5+ brings the penalty to the amount of wings in battle.
	...
	CAPITAL_RATIO_FOR_FULL_SCREENING_FOR_CONVOYS 					= 0.2,  -- this capital ratio to num convoys is needed for full screening beyond screen line
...
NAI = {
	...
	DIPLOMACY_ACCEPT_VOLUNTEERS_BASE = 200,		-- Base value of volunteer acceptance (help is welcome)
	DIPLOMACY_ACCEPT_ATTACHE_BASE = 200,			-- Base value of attache acceptance (help is welcome)
	...
	MAX_REQUEST_EXPEDITIONARIES_ARMY_RATIO = 0.0,				-- AI will not accept expeditionary requests if its expeditions are above this ratio
	...
	PROPOSE_LEND_LEASE_AIDESIRE_SAME_IDEOLOGY = 200,				-- Added to AI desire to propose lend lease if recipent is same ideology (and AI can't declare war on recipient)
	PROPOSE_LEND_LEASE_AIDESIRE_SAME_IDEOLOGY_CIVIL_WAR = 100,	-- Added to AI desire to propose lend lease if recipent is same ideology and they are currently in civil war
	SEND_VOLUNTEER_AIDESIRE_SAME_IDEOLOGY = -400,					-- Added to AI desire to send volunteers if recipent is same ideology (and AI can't declare war on recipient)
	SEND_VOLUNTEER_AIDESIRE_SAME_IDEOLOGY_CIVIL_WAR = -400,		-- Added to AI desire to send volunteers if recipent is same ideology and they are currently in civil war
...
NSupply = {
	...
	NAVAL_BASE_STARTING_PENALTY_PER_PROVINCE = 0.2,
	...
	NODE_FLOW_BONUS_PER_RAIL_LEVEL = 0.35,
	...
	INFRA_TO_SUPPLY = 0.40,							-- each level of infra gives this many supply
	...
	TRUCK_ATTRITION = 0.002, -- base truck attrition
